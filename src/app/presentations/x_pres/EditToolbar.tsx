...
export function EditToolbar({ sceneIndex, overrides, onUpdate, accentColor, onPositionPreset }: EditToolbarProps) {
  const current = overrides[sceneIndex] || {};
  const [activeSection, setActiveSection] = useState<"position" | "size" | "border" | "font" | null>(null);
  ...
  return (
    ...
    >
      ...
      <div className="x-pres-edit-tabs">
        {(["position", "size", "border", "font"] as const).map((section) => (
          <button
            key={section}
          ...
          >
            {section === "position" ? "📍" : section === "size" ? "↔" : section === "border" ? "▢" : "𝐀"}
          </button>
        ))}
      </div>
